...
class ELMLRF(object):
    ...
    def build(self, dtype=tf.float32, randseed=None):
        ...
        self._W['C1'] = tf.get_variable(
            name='C1', shape=[3, 3, self._insize[2], 32],
        ...
        self._W['C3'] = tf.get_variable(
            name='C3', shape=[3, 3, 32, 64],
            dtype=dtype, initializer=InitA, trainable=False)

        self._W['C4'] = tf.get_variable(
            name='C4', shape=[3, 3, 64, 64],
            dtype=dtype, initializer=InitA, trainable=False)

        self._W['C5'] = tf.get_variable(
            name='C5', shape=[3, 3, 64, 128],
            dtype=dtype, initializer=InitA, trainable=False)

        self._W['C6'] = tf.get_variable(
            name='C6', shape=[3, 3, 128, 256],
            dtype=dtype, initializer=InitA, trainable=False)

        self._W['C7'] = tf.get_variable(
            name='C7', shape=[4, 4, 256, 10000],
            dtype=dtype, initializer=InitA, trainable=False)
        ...
        # self._net['P5'] = tf.nn.max_pool(
        self._net['P4'] = tt.nn.square_root_pool(
            self._net['C4'],
            ksize=[1, 3, 3, 1],  # [1, kH, kW, 1]
            strides=[1, 1, 1, 1],  # [1, sH, sW, 1]
            padding="VALID", data_format='NHWC', name='P4')

        self._net['C5'] = tf.nn.conv2d(
            self._net['P4'], self._W['C5'],
            strides=[1, 1, 1, 1],  # [1, sH, sW, 1]
            padding="VALID", data_format='NHWC',
            dilations=[1, 1, 1, 1], name='C5')

        self._net['C5'] = tf.nn.swish(self._net['C5'])

        # self._net['P3'] = tf.nn.max_pool(
        self._net['P5'] = tt.nn.square_root_pool(
            self._net['C5'],
            ksize=[1, 3, 3, 1],  # [1, kH, kW, 1]
            strides=[1, 1, 1, 1],  # [1, sH, sW, 1]
            padding="VALID", data_format='NHWC', name='P5')

        self._net['C6'] = tf.nn.conv2d(
            self._net['P5'], self._W['C6'],
            strides=[1, 1, 1, 1],  # [1, sH, sW, 1]
            padding="VALID", data_format='NHWC',
            dilations=[1, 1, 1, 1], name='C6')

        self._net['C6'] = tf.nn.swish(self._net['C6'])

        # self._net['P3'] = tf.nn.max_pool(
        self._net['P6'] = tt.nn.square_root_pool(
            self._net['C6'],
            ksize=[1, 3, 3, 1],  # [1, kH, kW, 1]
            strides=[1, 1, 1, 1],  # [1, sH, sW, 1]
            padding="VALID", data_format='NHWC', name='P6')

        self._net['C7'] = tf.nn.conv2d(
            self._net['P6'], self._W['C7'],
            strides=[1, 1, 1, 1],  # [1, sH, sW, 1]
            padding="VALID", data_format='NHWC',
            dilations=[1, 1, 1, 1], name='C7')

        self._net['C7'] = tf.nn.swish(self._net['C7'])
        ...
        self._net['H'] = tf.reshape(HO, shape=[-1, 10000], name='H')
